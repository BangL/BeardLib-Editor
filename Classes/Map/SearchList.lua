...
function SearchList:init(parent)
    self._menu = parent._holder:pan("Tab",
        { visible = false, auto_height = false, stretch_to_bottom = true, scrollbar = false })

    self._options = self._menu:group("Options")
    local search = self._options:textbox("Search", ClassClbk(self, "do_search", false, false), nil,
        { textbox_offset = 22, control_slice = 0.75 })
    local h = self._options:ItemsHeight(1)
    search:tb_imgbtn("ClearSearch", ClassClbk(self, "clear_search"), nil, BLE.Utils.EditorIcons.cross,
        { w = h, h = h, offset = 0, img_scale = 0.5, position = "RightCentery" })

    self._parent = parent
    self._pages_panel = self._menu:holder("Pages",
        {
            align_method = "centered_grid",
            auto_align = false,
            h = 32,
            size = self._menu.size * 0.9,
    ...
    self._list = self._menu:pan("Units",
        { inherit_values = { size = self._menu.size * 0.8 }, auto_align = false, auto_height = false, stretch_to_bottom = true })
    self:do_search()
end
...
function SearchList:do_show(no_clear)
    ...
    local back = self._pages_panel:tb_imgbtn("Backwards", ClassClbk(self, "set_page", self._page - 1), nil,
        icons.arrow_left, popt)
    ...
    for i, page_num in pairs(pagination) do
        ...
        self._pages_panel:button(page_num, ClassClbk(self, "set_page", page_num),
            { border_bottom = page_num == page, h = h, w = 32, text_align = "center" })
    end

    self._pages_panel:tb_imgbtn("Forward", ClassClbk(self, "set_page", self._page + 1), nil, icons.arrow_right, popt)
        :SetEnabled(self._page ~= pages and pages > 1)
